...
(function () {
    ...
    // Получаем уникальный номер вкладки
    function getTabNumber() {
        let tabs = JSON.parse(localStorage.getItem('mouse-tabs') || '[]');
        // Удаляем старые вкладки (больше 10 минут)
        const now = Date.now();
        tabs = tabs.filter(t => now - t.time < 600000);
        // Если уже есть, возвращаем
        let found = tabs.find(t => t.id === window.name);
        if (!found) {
            window.name = window.name || String(Math.random());
            tabs.push({id: window.name, time: now});
            localStorage.setItem('mouse-tabs', JSON.stringify(tabs));
        }
        // Обновляем время активности
        tabs = tabs.map(t => t.id === window.name ? {id: t.id, time: now} : t);
        localStorage.setItem('mouse-tabs', JSON.stringify(tabs));
        return tabs.findIndex(t => t.id === window.name);
    }

    function updateTabStatus() {
        tabNumber = getTabNumber();
        // Активная вкладка — та, у которой tabNumber === 0
        activetab = (tabNumber === 0);
    }
